...
def calc_psnr_and_ssim(sr, hr):
    ...
    for i in range(sr.size()[0]):
        img1 = np.transpose(sr[i].squeeze().round().cpu().numpy(), (1,2,0))
        img2 = np.transpose(hr[i].squeeze().round().cpu().numpy(), (1,2,0))
...
def load_model(model, model_path, data_parallel=False):
    if os.path.exists(model_path):
        model_state_dict_save = {k.replace('module.',''):v for k,v in torch.load(model_path).items()}
        if data_parallel:
            model_state_dict_save = {'module.' + k: v for k, v in torch.load(model_path).items()}
